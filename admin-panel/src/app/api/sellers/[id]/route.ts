...
export async function GET(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    const response = await axios.get(
      `${process.env.NEXT_PUBLIC_API_URL}/api/v1/sellers/${id}`,
      {
        headers: {
    ...
    console.error('Get seller error:', error.response?.data || error.message);
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('Update seller error:', error.response?.data || error.message);
